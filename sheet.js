...
function applyOperation() {
    ...
    const filteredData = excelData.filter((row, index) => {
        ...
        // Validate each operation column based on content type
        return operationColumns.every(col => {
            ...
            switch (contentType) {
                case 'all':
                    ...
                    break;
                case 'word':
                    // Only words (A-Z, a-z) are set as not null
                    isNotNull = /^[A-Za-z]+$/.test(cellValue);
                    break;
                case 'number':
                    // Valid if it's a number
                    isNotNull = !isNaN(cellValue) && cellValue !== "";
                    break;
                case 'link':
                    // Valid if it's a URL
                    isNotNull = /^(https?:\/\/[^\s]+)/.test(cellValue);
                    break;
            }

            // For null, check if not null is false
            return isNotNull === (contentType !== 'null');
        }) && !isPrimaryNull;
    });
